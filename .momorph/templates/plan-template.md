...
- [ ] **I. Clean Code & Source Organization** — feature-first package layout under
      `com.example.aiddproject.<feature>/{ui,data,domain}`; composables <150 LOC; Kotlin official
      style
- [ ] **II. Tech Stack Best Practices** — Kotlin idioms (immutability, coroutines/Flow), Compose
      state hoisting, repository pattern, Supabase client uses anon key only, versions pinned via
      `libs.versions.toml`
- [ ] **III. Material Design 3 (Android)** — M3 components only, theme tokens, dynamic color +
      light/dark, accessibility (48dp targets, contentDescription, font scaling), responsive via
      `WindowSizeClass`
- [ ] **IV. OWASP Secure Coding** — no secrets in source; TLS + cert pinning; input validation;
      RLS enforced on every Supabase table touched; no PII/token logging; threat model included
      below if feature handles auth/PII/payments/uploads
- [ ] **V. Test-Driven Development** — failing unit/instrumented tests committed before
      implementation; RLS policy tests for new tables; no merge with untested public functions in
      `domain/` or `data/`
