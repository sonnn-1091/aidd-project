...
| 10 | [iOS] Sun*Kudos | fO0Kt19sZZ | https://www.figma.com/design/9ypp4enmFmdK3YAFJLIu6C?node-id=fO0Kt19sZZ | spec_shipped (full spec, ratified 2026-05-12) | specs/fO0Kt19sZZ-iOS-Sun-Kudos/spec.md | Home (Sun*Kudos Chi tiết / bottom-nav Kudos tab), Award Detail (Sun*Kudos Chi tiết) | Viết Kudo (7fFAb-K35a) via Send pill / compose FAB, Send Kudos compose (PV7jBVZU1N), Kudo Detail (T0TR16k0vH / 5C2BL6GYXL anonymous), All Kudos paginated (j_a2GQWKDJ), Open Secret Box (kQk65hSYF2), Profile bản thân (hSH7L8doXB) / Profile người khác (bEpdheM0yU), Login (8HGlvYGJWq on 401). Hashtag dropdown (V5GRjAdJyb) + Phòng ban dropdown (76k69LQPfj) as overlays. 4 Q-K resolved + Q-K-1 left as implementer-call. |
| 11 | [iOS] Sun*Kudos_Viết Kudo_default | 7fFAb-K35a | https://www.figma.com/design/9ypp4enmFmdK3YAFJLIu6C?node-id=7fFAb-K35a | spec_draft | specs/7fFAb-K35a-iOS-Sun-Kudos-Viet-Kudo/spec.md | Sun*Kudos hub (fO0Kt19sZZ) — Send pill in hero / compose FAB; Home (OuH1BUTYT0) — compose FAB (pencil icon, per TC_VIETKUDO_ACC_004); Search Sunner (3jgwke3E8O) — tap user row to prefill recipient | Sun*Kudos hub (fO0Kt19sZZ on submit success — triggers hub refresh; or back to whichever screen was the entry on Cancel), Login (8HGlvYGJWq on 401), Community Standards page (via B.5 "Tiêu chuẩn cộng đồng" link). Sub-flows: Lỗi chưa điền hết (0le8xKnFE_), dropdown tên người nhận (5MU728Tjck), dropdown hashtag (aKWA2klsnt). |
| 12 | [iOS] Sun*Kudos_Lỗi chưa điền hết | 0le8xKnFE_ | https://www.figma.com/design/9ypp4enmFmdK3YAFJLIu6C?node-id=0le8xKnFE_ | sub_flow | — (no separate spec; covered in parent 7fFAb-K35a) | Viết Kudo (7fFAb-K35a) — surfaced when user taps "Send" without all required fields | Returns to Viết Kudo (7fFAb-K35a) after the user dismisses / fills missing fields. |
| 13 | [iOS] Sun*Kudos_Gửi lời chúc Kudos_dropdown tên người nhận | 5MU728Tjck | https://www.figma.com/design/9ypp4enmFmdK3YAFJLIu6C?node-id=5MU728Tjck | sub_flow | — (no separate spec; covered in parent 7fFAb-K35a) | Viết Kudo (7fFAb-K35a) — recipient-name dropdown overlay | Returns to Viết Kudo (7fFAb-K35a) on selection / dismiss. |
| 14 | [iOS] Sun*Kudos_Gửi lời chúc Kudos_dropdown hashtag | aKWA2klsnt | https://www.figma.com/design/9ypp4enmFmdK3YAFJLIu6C?node-id=aKWA2klsnt | sub_flow | — (no separate spec; covered in parent 7fFAb-K35a) | Viết Kudo (7fFAb-K35a) — hashtag dropdown overlay | Returns to Viết Kudo (7fFAb-K35a) on selection / dismiss. |
| 15 | [iOS] Sun*Kudos_Tiêu chuẩn cộng đồng | xms7csmDhD | https://www.figma.com/design/9ypp4enmFmdK3YAFJLIu6C?node-id=xms7csmDhD | spec_draft | specs/xms7csmDhD-iOS-Sun-Kudos-Tieu-chuan-cong-dong/spec.md | Viết Kudo (7fFAb-K35a) — "Tiêu chuẩn cộng đồng" link in formatting toolbar row (B.5); reusable from any "Community Standards" link elsewhere in Sun*Kudos (no other entry points currently identified) | Returns BACK to Viết Kudo (7fFAb-K35a) on dismiss — no downstream screens. Content sections: "Tiêu chuẩn cộng đồng" (10 anti-spam criteria) + "Tiêu chuẩn bảo mật" (security standards + Slack support contact); top-nav back button is the only interactive element. Static, no APIs. |
| 16 | [iOS] Sun*Kudos_Search Sunner | 3jgwke3E8O | https://www.figma.com/design/9ypp4enmFmdK3YAFJLIu6C?node-id=3jgwke3E8O | spec_draft | specs/3jgwke3E8O-iOS-Sun-Kudos-Search-Sunner/spec.md | Viết Kudo (7fFAb-K35a) — recipient search entry (tap recipient input or search affordance); likely also entered from global Search action / Home (OuH1BUTYT0) search bar / Sun*Kudos hub (fO0Kt19sZZ) search. Related sibling state: [iOS] Sun*Kudos_Searching (hldqjHoSRH) is the type-ahead / intermediate searching state — noted here for context, not yet specced. | Returns to Viết Kudo (7fFAb-K35a) on user-row tap — prefills the recipient field with the tapped Sunner (per the Viết Kudo spec contract). Dismiss / back returns to the entry screen without mutation. Search-results screen for finding Sunners (employees/colleagues); predicted API `GET /sunners?search=` for the result list. |
| 17 | [iOS] Notifications | _b68CBWKl5 | https://www.figma.com/design/9ypp4enmFmdK3YAFJLIu6C?node-id=_b68CBWKl5 | spec_draft | specs/_b68CBWKl5-iOS-Notifications/spec.md | Home (OuH1BUTYT0) — bell icon in `HomeHeader` (`onBellClick`); Award Detail (c-QM3_zjkG) — same shared `HomeHeader` bell; Sun*Kudos hub (fO0Kt19sZZ) — same shared `HomeHeader` bell. All three entry screens already wire an `onBellClick` callback through their `HomeHeader` composable, so wiring the route is a 1-line change per call site. | Back to caller via top-nav back arrow (preserves caller's scroll / state — standard Compose `popBackStack`). Per-notification tap likely deep-links to the referenced detail screen (Kudo Detail T0TR16k0vH / 5C2BL6GYXL anonymous, Award Detail c-QM3_zjkG, Profile bản thân hSH7L8doXB / Profile người khác bEpdheM0yU, etc.) — exact targets pending spec. Login (8HGlvYGJWq) on 401. Predicted APIs: `GET /notifications` (list, paginated), `PUT /notifications/:id/read` (mark single read), `PUT /notifications/read-all` (mark-all-read), `GET /notifications/unread-count` (bell badge — likely also called from each entry screen on resume). |
